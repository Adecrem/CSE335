 ...
 *
 * Factory that draws the first machine
 *
 */
...
/**
 * Create the post in which the pullys sit on
 * \param x
 * \param y
 * \param wid
 * \param hit
 ...
 * \returns
 */
std::shared_ptr<CShape> CMachineOneFactory::CreatePost(int x, int y, int wid, int hit, Gdiplus::Color color)
{
...
/**
 * Create working machine that actually draws and functions
 * Given on the website, I basically designed my entire project around this code.
 * \returns 
 */
std::shared_ptr<CBlower> CMachineOneFactory::Create()
{
        // The machine itself
        auto machine = make_shared<CBlower>(1);
        ...
        // The motor
        //
        auto motor = make_shared<CMotor>(50);
